...
import { ApiResponse, Department } from '../types/api.types';
...
export class DepartmentService {
    ...
    fetchAll(
        ...
        return this.api.get<ApiResponse<Department[]>>(this.endpoint, params).pipe(
            tap({
                next: (res) => {
                    if (append) {
                        this._departments.update((prev) => [...prev, ...res.data]);
                    } else {
    ...
    create(payload: DepartmentPayload) {
        return this.api.post<any>(this.endpoint, payload).pipe(
            tap((res) => {
                ...
                if (newPos) {
                    this._departments.update((list) => [...list, newPos]);
                }
            }),
        );
    }
    ...
    remove(id: string) {
        return this.api.delete<any>(`${this.endpoint}/${id}`).pipe(
            tap(() => {
                this._departments.update((list) => list.filter((p) => p.id !== id));
            }),
        );
    }
}
